...
class ArticleCollectionTest extends \PHPUnit_Framework_TestCase
{
    public function testConstruct()
    {
        $uuid = Uuid::uuid4();
        $now = new \DateTime();
        $art = new Article(new Url('http://localhost'), 'title', 'content', 'content');
    ...
    {
        $art1 = new Article(new Url('http://localhost'), 'title', 'content1', 'content1');
        $art2 = new Article(new Url('http://localhost'), 'title', 'content2', 'content2');
        $aco = new ArticleCollection(
            Uuid::uuid4(),
            new \DateTime(),
            'title',
